	...
	public String onEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("buy_scroll_24"))
		{
			...
			else
			{
				...
				if (hours > 0)
				{
					final SystemMessage sm = new SystemMessage(SystemMessageId.THERE_ARE_S1_HR_AND_S2_MIN_REMAINING_UNTIL_THE_ITEM_CAN_BE_OBTAINED_AGAIN);
					sm.addInt(hours);
					sm.addInt(minutes);
				...
				{
					final SystemMessage sm = new SystemMessage(SystemMessageId.THERE_ARE_S1_MIN_REMAINING_UNTIL_THE_ITEM_CAN_BE_OBTAINED_AGAIN);
					sm.addInt(minutes);
					player.sendPacket(sm);
					htmltext = "32599-scroll24.htm";
				}
