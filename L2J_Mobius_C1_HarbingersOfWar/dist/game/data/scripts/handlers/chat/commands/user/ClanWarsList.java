...
public class ClanWarsList implements IUserCommandHandler
{
	...
	public boolean onCommand(int id, Player player)
	{
		...
		try (Connection con = DatabaseFactory.getConnection())
		{
			...
			if (id == 88)
			{
				player.sendMessage("=======<Attack List>=======");
				query = ATTACK_LIST;
			}
			// Under Attack List
			else if (id == 89)
			{
				player.sendMessage("======<Under Attack List>======");
				query = UNDER_ATTACK_LIST;
			}
			// War List
			else
			{
				player.sendMessage("=====<War List>=====");
				query = WAR_LIST;
			}
			...
			try (PreparedStatement ps = con.prepareStatement(query))
			{
				...
				try (ResultSet rs = ps.executeQuery())
				{
					...
					while (rs.next())
					{
						...
						{
							// Target With Ally
							player.sendMessage("=" + clanName + " (" + rs.getString("ally_name") + " alliance)");
						}
						else
						{
							// Target Without Ally
							player.sendMessage("=" + clanName + " (No alliance exists.)");
						}
					}
				}
			}
