 ...
 */
package org.l2jmobius.gameserver.mechanics.siege.clanhalls;
...
public abstract class ClanHallSiegeEngine extends Script implements Siegable
{
	...
	public void startSiege()
	{
		if ((_attackers.isEmpty()) && (_hall.getId() != 21)) // Fortress of resistance don't have attacker list.
		{
			...
			_hall.updateSiegeStatus(SiegeStatus.WAITING_BATTLE);
			World.broadcastToAllOnlinePlayers("The siege of " + _hall.getName() + " has been canceled due to lack of interest.");
			return;
		}
	...
	public void endSiege()
	{
		...
		World.broadcastToAllOnlinePlayers(end);
		...
		if (_missionAccomplished && (winner != null))
		{
			...
			finalMsg = "Clan " + winner.getName() + " is victorious over " + _hall.getName() + "'s castle siege!";
		}
		else
		{
			finalMsg = "The siege of " + _hall.getName() + " has ended in a draw.";
		}
		World.broadcastToAllOnlinePlayers(finalMsg);
