 ...
 */
package org.l2jmobius.gameserver.entity.groups;
...
public class Party extends AbstractPlayerGroup
{
	 ...
	 * @see org.l2jmobius.gameserver.entity.actor.Player#isRequestExpired()
	 */
	...
	public void broadcastToPartyMembers(Player player, String text)
	{
		for (Player member : _members)
		{
			if ((member != null) && (member.getObjectId() != player.getObjectId()))
	...
	public void removePartyMember(Player player, PartyMessageType type)
	{
		if (_members.contains(player))
		{
			...
			{
				LOGGER.log(Level.WARNING, "removePartyMember: ", e);
			}
			
			if (type == PartyMessageType.EXPELLED)
			{
				player.sendPacket(SystemMessageId.YOU_HAVE_BEEN_EXPELLED_FROM_THE_PARTY);
				final SystemMessage msg = new SystemMessage(SystemMessageId.S1_WAS_EXPELLED_FROM_THE_PARTY);
				msg.addString(player.getName());
				broadcastPacket(msg);
			}
			...
			{
				broadcastString(getLeader().getName() + " has become a party leader.");
	...
	public void setLeader(Player player)
	{
		...
		if (_members.contains(player))
		{
			if (isLeader(player))
			{
				player.sendMessage("You cannot transfer rights to yourself.");
			}
			else
			{
				// Swap party members
				...
				if (isInCommandChannel() && _commandChannel.isLeader(temp))
				{
					...
					_commandChannel.broadcastString("The authority for command channel has been transferred to " + _commandChannel.getLeader().getName() + ".");
				}
			}
		}
		else
		{
			player.sendMessage("You can transfer rights only to another party member.");
		}
	}
	...
	public void distributeItem(Player player, Item item)
	{
		...
		if (item.getCount() > 1)
		{
			final SystemMessage msg = new SystemMessage(SystemMessageId.S1_PICKED_UP_S3_S2);
	...
	public void distributeItem(Player player, int itemId, int itemCount, boolean spoil, Attackable target)
	{
		...
		if (itemCount > 1)
		{
			if (spoil)
			{
				broadcastToPartyMembers(looter, looter.getName() + " picked up " + itemCount + " " + ItemData.getInstance().getTemplate(itemId).getName() + " by usinf sweeper.");
			}
			else
			{
				final SystemMessage msg = new SystemMessage(SystemMessageId.S1_PICKED_UP_S3_S2);
		...
		else
		{
			if (spoil)
			{
				broadcastToPartyMembers(looter, looter.getName() + " picked up " + ItemData.getInstance().getTemplate(itemId).getName() + " by using swwper.");
			}
			else
			{
				final SystemMessage msg = new SystemMessage(SystemMessageId.S1_PICKED_UP_S2);
	...
	public void distributeAdena(Player player, int adena, Creature target)
	{
		...
		final List<Player> toReward = new ArrayList<>();
