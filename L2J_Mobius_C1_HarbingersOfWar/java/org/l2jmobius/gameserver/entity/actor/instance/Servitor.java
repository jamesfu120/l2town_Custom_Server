...
public class Servitor extends Summon implements Runnable
{
	...
	{
		if (!PlayerConfig.SUMMON_STORE_SKILL_COOLTIME || (getOwner() == null))
		{
			return;
		}
	...
	public void run()
	{
		...
		{
			sendMessage("Servitor passed away.");
			unSummon(getOwner());
			return;
		}
		...
		if (_consumeItemInterval > 0)
		{
			...
			if ((_consumeItemIntervalRemaining <= 0) && (_itemConsume.getCount() > 0) && (_itemConsume.getId() > 0) && !isDead())
			{
				...
				{
					sendMessage("A summoned monster uses " + ItemData.getInstance().getTemplate(_itemConsume.getId()).getName() + ".");
				...
				{
					sendMessage("Since you do not have enough items to maintain the servitor's stay, the servitor will disappear.");
					unSummon(getOwner());
				}
			}
		}
