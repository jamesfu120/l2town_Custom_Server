...
{
	private final Map<PlayerClass, List<InitialEquipment>> _classEquipmentList = new EnumMap<>(PlayerClass.class);
	
	private static final String NORMAL = "data/stats/players/initialEquipment.xml";
	...
	public void load()
	{
		_classEquipmentList.clear();
		parseDatapackFile(PlayerConfig.INITIAL_EQUIPMENT_EVENT ? EVENT : NORMAL);
		LOGGER.info(getClass().getSimpleName() + ": Loaded " + _classEquipmentList.size() + " initial equipment data.");
	}
	 ...
	 * @param d parse an initial equipment and add it to {@link #_classEquipmentList}
	 */
	private void parseEquipment(Node d)
	{
		...
		final List<InitialEquipment> equipList = new ArrayList<>();
		for (Node c = d.getFirstChild(); c != null; c = c.getNextSibling())
		{
			if ("item".equalsIgnoreCase(c.getNodeName()))
			{
				...
				equipList.add(new InitialEquipment(set));
			}
		}
		
		_classEquipmentList.put(classId, equipList);
	}
	 ...
	 */
	public List<InitialEquipment> getEquipmentList(PlayerClass cId)
	{
		return _classEquipmentList.get(cId);
	}
	 ...
	 * @return the initial equipment for the given class Id.
	 */
	public List<InitialEquipment> getEquipmentList(int cId)
	{
		return _classEquipmentList.get(PlayerClass.getPlayerClass(cId));
	}
