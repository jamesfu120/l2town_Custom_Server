...
public class TriggerSkillByDamageReceived extends AbstractEffect
{
	...
	private void onDamageReceivedEvent(OnCreatureDamageReceived event)
	{
		...
		// Warg - TODO: Unhardcode.
		if (_skill.getSkillId() == 31493 /* Transient Instinct */)
		{
			if (event.getTarget().isSkillDisabled(SkillData.getInstance().getSkill(31494 /* Transient Instinct */, 1)) || //
				event.getTarget().isSkillDisabled(SkillData.getInstance().getSkill(31495 /* Transient Instinct */, 1)) || //
				event.getTarget().isSkillDisabled(SkillData.getInstance().getSkill(31496 /* Transient Instinct */, 1)) || //
				event.getTarget().isSkillDisabled(SkillData.getInstance().getSkill(31497 /* Transient Instinct */, 1)) || //
				event.getTarget().isSkillDisabled(SkillData.getInstance().getSkill(31498 /* Transient Instinct */, 1)))
			{
				return;
			}
		}
