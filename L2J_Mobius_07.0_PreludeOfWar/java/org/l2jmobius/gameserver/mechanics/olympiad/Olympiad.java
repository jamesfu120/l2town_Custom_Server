...
public class Olympiad extends ListenersContainer
{
	...
	private static final String OLYMPIAD_GET_LEGEND = "SELECT olympiad_nobles.charId FROM olympiad_nobles WHERE olympiad_nobles.competitions_done >=" + OlympiadConfig.OLYMPIAD_MIN_MATCHES + " ORDER BY olympiad_nobles.olympiad_points DESC";
	...
	private void updateCompStatus()
	{
		...
		_scheduledCompStart = ThreadPool.schedule(() ->
		{
			...
			World.broadcastToAllOnlinePlayers(new SystemMessage(SystemMessageId.THE_OLYMPIAD_HAS_BEGAN));
			...
			_scheduledCompEnd = ThreadPool.schedule(() ->
			{
				...
				_inCompPeriod = false;
				World.broadcastToAllOnlinePlayers(new SystemMessage(SystemMessageId.BATTLES_IN_THE_OLYMPIAD_GAMES_ARE_NOW_OVER));
